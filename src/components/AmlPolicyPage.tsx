...
import { AML_POLICY_ARTICLES } from './amlPolicyArticles';
...
export function AmlPolicyPage() {
  ...
  const visibleArticles = useMemo(() => {
    ...
    if (!q) return AML_POLICY_ARTICLES;
    return AML_POLICY_ARTICLES.filter(
      (a) =>
        ...
        <div className="relative flex min-h-[25vh] flex-col items-center justify-center px-8 pb-20 pt-32 text-center md:min-h-[25vh] md:pb-28 md:pt-36">
          ...
          <div className="relative z-10 max-w-3xl">
            <p className="mb-3 text-xs font-semibold uppercase tracking-[0.2em] text-white/80">
              Zena Technologies Limited
            </p>
            <h1 className="text-[clamp(1.75rem,5vw,3rem)] font-bold tracking-tight text-white">
              Anti-Money Laundering Policy
            </h1>
            <p className="mt-4 text-sm text-white/65 md:text-base">
              Approved by: MD/CEO · Date of Development: 08 March 2026 · Developed by: Legal &amp; Compliance
            </p>
          </div>
        </div>
      </section>
